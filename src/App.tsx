...
export default function App() {
  ...
  const [celebrate, setCelebrate] = useState<{ level: 'small' | 'big'; key: number } | null>(null)
  ...
  // Which side of the stage the student can touch this beat.
  ...
  // Enter a beat: seed the live bar, speak the line, fire any celebration.
  useEffect(() => {
    if (!started) return
    clearTimer()
    if (interactiveSpec) {
      setLive({ ...(interactiveSpec as BarSpec), ownerBeat: beatId })
    }
  ...
  // Resolve a stage slot to a concrete bar (handles 'keep' + live interactivity).
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-space-900 font-sans">
      ...
      <AnimatePresence>
        {!started && <StartScreen onBegin={begin} />}
      </AnimatePresence>

      {started && (
        <>
          {/* top chrome: pause + phase pips */}
            ...
            <div className="flex items-center gap-2">
              {PHASES.map((p, i) => (
                <div key={p} className="flex items-center gap-2">
                  ...
                  {i < PHASES.length - 1 && (
                    <span className="h-px w-4 bg-cream/15 sm:w-7" />
                  )}
                </div>
              ))}
            </div>
          ...
          {/* main: tutor (left) + scene (right). Stacks on narrow screens. */}
